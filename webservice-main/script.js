const ApiKey = "92c2ed00d54e43e1ad646182ca77b22e";
...
const detailEndPoin = `${baseUrl}competitions/${leagueId}/team`;
...
function showTeamInfo(id) {
    ...
    fetch(url, fetchHeader)
        .then(response => response.json())
        .then(detailTim => {
            console.log(detailTim);
            let detail = `
                    <img src="${detailTim.crestUrl}" alt="${detailTim.name}" width="50px"
                <div class="card-content">
                    <span class="card-title
                        activator grey-text text-darken-4">
                        ${detailTim.name}
                    </span>
                    <p> Berdiri: ${detailTim.founded} <br>
                        Stadion: ${detailTim.venue}
                    </p>
                </div>
            `;
